...
先说清楚一件事——deusyu 整理的这套，对 OpenAI Codex 那种百万行代码、十几个 agent 并行、几十人团队协作的场景，**是对的**。

我反对的不是 deusyu 写错了。是这套**根本不是写给我的**。

我是一个人对着 Claude 写代码、同时维护四五个小项目的开发者。下面四条原则——逐条反对。
...
> **业界几乎所有 harness 讨论都缺第二层。**

OpenAI 的 sandbox 里没有"开发者本人的认知存档"这个位置；Anthropic 的 CLAUDE.md 只能存命令和规则，存不下你的设计哲学。
...
```
┌──────────┐   ┌──────────────┐   ┌────────────────┐   ┌─────────────────┐
│ 你犯一个错 │ → │  Claude 修   │ → │ 沉淀进 CLAUDE.md │ → │ 下次会话自动防御 │
└──────────┘   └──────────────┘   └────────────────┘   └─────────────────┘
       ▲                                                          │
       └──────────────────────────────────────────────────────────┘
                  （犯下一个错时，规则已就位 → 不会再踩）
...
```
人层（~/Brain/）                    机器层（Claude context）
┌──────────────────┐                ┌──────────────────┐
│  项目笔记         │ ─SessionStart─▶│  会话上下文       │
│  经验教训         │                │  工具调用         │
│  日报             │ ◀──/wrap-up── │                  │
└──────────────────┘                └──────────────────┘
        ▲                                    │

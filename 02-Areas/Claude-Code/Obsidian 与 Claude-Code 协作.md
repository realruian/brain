...
**什么时候建？** 当你开始为一件事花超过一次对话的精力时就该建，不需要等代码出现：

| 阶段   | 触发信号               | 笔记内容                          |
| ---- | ------------------ | ----------------------------- |
| 想法阶段 | 脑子里在转，但还没动手        | 这东西是什么、解决什么问题、初步技术方向、还没想清楚的问题 |
| 调研阶段 | 在看文档、对比方案、问 Claude | 对比了哪些方案、选了哪个、为什么、踩到的坑、参考链接    |
| 准备动手 | 知道要做什么，代码还没写       | 确定的技术栈和架构、核心模块划分、第一步做什么       |
| 已有代码 | 代码仓库已存在            | 项目目标、技术栈、核心架构、已知限制            |

无代码时：

```
帮我在 01-Projects 里建个项目笔记，项目叫 XX，目前还在想法阶段，我想做的是……
```
...
项目笔记里的技术栈、架构、已知限制不是写一次就完的快照。发生以下变化时**立刻同步**，否则下次对话会基于过时信息给方案：

- 技术栈变了（引入新库、替换旧方案）
- 架构调整了（模块拆分、数据流重设计）
- 发现新的硬约束（API 限流、浏览器兼容、性能瓶颈）

```
把刚才的架构调整更新到 ~/Brain/01-Projects/xx.md 里
```

日常迭代（加组件、修 bug、小重构）不需要每次同步，留给 3.5 随手记录和 4.3 收工同步处理。
